...
# 粗糙地形 + Orbbec 仿真导航启动脚本
# 在桌面环境中使用 gnome-terminal 多窗口启动；在容器中自动退化为后台进程 + 日志文件。
# 容器模式下每个任务使用独立进程组，Ctrl-C 会清理其 ROS/Tk 子进程。

LOG_DIR="/tmp/lidar_nav2_logs/nav2_rough_sim_$(date +%Y%m%d_%H%M%S)"
PID_FILE="/tmp/lidar_nav2_logs/.nav2_rough_sim.pgids"
PGIDS=()
USE_TERMINAL=0
ENABLE_TELEOP=0
STOP_PATTERNS=(
...
stop_workspace_gazebo() {
  local world_path="$WORKSPACE_ROOT/install/get_urdf/share/get_urdf/worlds/rough_terrain.world"
  pkill -TERM -f "$world_path" 2>/dev/null || true
  pkill -TERM -f "get_urdf/share/get_urdf/worlds/3d.world" 2>/dev/null || true
  pkill -TERM -f "/tmp/lidar_nav2_rough_terrain.world" 2>/dev/null || true
  sleep 0.5
  pkill -KILL -f "$world_path" 2>/dev/null || true
  pkill -KILL -f "get_urdf/share/get_urdf/worlds/3d.world" 2>/dev/null || true
  pkill -KILL -f "/tmp/lidar_nav2_rough_terrain.world" 2>/dev/null || true
}
...
cleanup() {
  trap - INT TERM EXIT
  if [ "${#PGIDS[@]}" -gt 0 ]; then
    echo "Stopping nav2_sim processes..."
...
run_task "Gazebo 仿真" "killall -9 gzserver gzclient 2>/dev/null || true; ros2 launch ./scripts/launch_rough_gazebo.py"
...
run_task "Orbbec地形识别" "ros2 launch terrain_perception terrain_perception_launch.py use_sim_time:=true"
